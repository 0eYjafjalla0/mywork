...
LTE_config.N_RB_UL = 0; % 初始化上行资源块数量
LTE_config.N_RB_DL = 0; % 初始化下行资源块数量
...
% 如果设置了上下行资源块数量，则使用设置的值
if isfield(LTE_config, 'N_RB_UL') && isfield(LTE_config, 'N_RB_DL')
    if LTE_config.N_RB_UL + LTE_config.N_RB_DL > LTE_config.N_RB
        error('Sum of UL and DL resource blocks exceeds total resource blocks');
    end
    N_RB_UL = LTE_config.N_RB_UL;
    N_RB_DL = LTE_config.N_RB_DL;
else
    % 默认上下行资源块数量相等
    N_RB_UL = floor(LTE_config.N_RB / 2);
    N_RB_DL = LTE_config.N_RB - N_RB_UL;
end

% 根据上下行资源块数量设置相关参数
LTE_config.N_RB_UL = N_RB_UL;
LTE_config.N_RB_DL = N_RB_DL;
LTE_config.Ntot_UL = N_RB_UL * 12; % 上行总子载波数量
LTE_config.Ntot_DL = N_RB_DL * 12; % 下行总子载波数量
LTE_config.fs_UL = 15e3 * LTE_config.fft_points * N_RB_UL / LTE_config.N_RB; % 上行采样频率
LTE_config.fs_DL = 15e3 * LTE_config.fft_points * N_RB_DL / LTE_config.N_RB; % 下行采样频率
